...
bun-browser site search <query>

# 运行 adapter（简写，推荐）
bun-browser site <name> [args...]
...
<!-- 证据来源：README.md "36 platforms, 103 commands"，具体 adapter 数量以 bun-browser site list 实际输出为准 -->
...
bun-browser site reddit/search "local LLM"
bun-browser site hackernews/top

# 查看热榜
bun-browser site zhihu/hot
bun-browser site weibo/hot
bun-browser site v2ex/hot

# 查询股票
bun-browser site eastmoney/stock "茅台"
```
...
```bash
# GitHub 操作
bun-browser site github/repo anthropics/claude-code
bun-browser site github/issues owner/repo

# 查包信息
bun-browser site npm/package bun-browser
bun-browser site pypi/package requests

# 搜索技术问题
bun-browser site stackoverflow/search "async await error handling"
```
...
# 论文搜索
bun-browser site arxiv/search "transformer attention mechanism"
```
...
```bash
bun-browser site zhihu/hot --json
bun-browser site twitter/search "AI" --json
```
