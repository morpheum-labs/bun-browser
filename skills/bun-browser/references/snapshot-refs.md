...
bun-browser 使用紧凑的元素引用（ref），大幅减少 AI Agent 的上下文消耗。
...
# 填写邮箱
bun-browser fill @10 "user@example.com"

# 填写密码
bun-browser fill @11 "password123"

# 提交表单
bun-browser click @12
```
...
bun-browser snapshot -i          # 先获取 ref
bun-browser click @1             # 再使用

# 错误
bun-browser open https://example.com
bun-browser click @1             # ref 不存在！
```

### 2. 导航后重新 Snapshot

```bash
bun-browser click @5             # 导航到新页面
bun-browser snapshot -i          # 获取新页面的 ref
bun-browser click @1             # 使用新的 ref
```

### 3. 动态变化后重新 Snapshot

```bash
bun-browser click @1             # 打开下拉菜单
bun-browser snapshot -i          # 获取菜单项的 ref
bun-browser click @7             # 选择菜单项
```

### 4. 等待加载完成

```bash
bun-browser click @3             # 触发 AJAX 加载
bun-browser wait 1000            # 等待加载
bun-browser snapshot -i          # 获取新内容的 ref
```
...
```bash
bun-browser eval "document.querySelector('.hidden-element').click()"
```
