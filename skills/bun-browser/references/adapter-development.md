...
bun-browser network requests "api" --with-body --json --tab <tabId>
```
...
/* @meta
{
  ...
  "readOnly": true,
  "example": "bun-browser site reddit/search 'local LLM'"
}
*/
async function(args) {
...
```bash
# 保存到私有目录
# 文件路径：~/.bun-browser/sites/platform/command.js

# 测试运行
bun-browser site platform/command "test query" --json

# 验证输出格式
bun-browser site platform/command "test query"
```
