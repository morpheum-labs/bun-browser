...
你已经登录了微博、知乎、B站、小红书、Twitter、GitHub、LinkedIn — bun-browser 让 AI Agent **直接用你的登录态**。

```bash
bun-browser site twitter/search "AI agent"       # 搜索推文
bun-browser site zhihu/hot                        # 知乎热榜
bun-browser site arxiv/search "transformer"       # 搜论文
bun-browser site eastmoney/stock "茅台"            # 实时股票行情
bun-browser site boss/search "AI 工程师"           # 搜职位
bun-browser site wikipedia/summary "Python"       # 维基百科摘要
bun-browser site youtube/transcript VIDEO_ID      # YouTube 字幕全文
...
bun-browser 翻转了这个逻辑：**不是让网站适配机器，而是让机器使用人的界面。** adapter 在你的浏览器 tab 里跑 `eval`，用你的 Cookie 调 `fetch()`，或者直接调用页面的 webpack 模块。网站以为是你在操作。因为**就是你**。

| | Playwright / Selenium | 爬虫库 | bun-browser |
|---|---|---|---|
...
```bash
bun-browser site update        # 拉取社区适配器
bun-browser site recommend     # 看看哪些和你的浏览习惯匹配
bun-browser site zhihu/hot     # 开搞
```

### OpenClaw（无需安装扩展）

如果你使用 [OpenClaw](https://openclaw.ai)，bun-browser 可以直接通过 OpenClaw 内置浏览器运行，不需要额外安装 Chrome 扩展或 daemon：
...
没有 bun-browser，AI Agent 的世界是：**文件系统 + 终端 + 少数有 API key 的服务。**

有了 bun-browser：**文件系统 + 终端 + 整个互联网。**
...
bun-browser snapshot -i                # 可访问性树
bun-browser click @3                   # 点击元素
bun-browser fill @5 "hello"            # 填写输入框
bun-browser eval "document.title"      # 执行 JS
bun-browser fetch URL --json           # 带登录态的 fetch
bun-browser network requests --with-body --json  # 抓包
bun-browser screenshot                 # 截图
```
...
bun-browser site info xueqiu/stock   # 查看 adapter 参数、示例、域名
```
...
bun-browser daemon --host 0.0.0.0      # 监听所有网卡（用于 Tailscale / ZeroTier 跨机器访问）
```
