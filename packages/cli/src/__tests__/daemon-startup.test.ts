...
const DAEMON_DIR = path.join(os.tmpdir(), `bun-browser-test-startup-${process.pid}`);
mkdirSync(DAEMON_DIR, { recursive: true });
process.env.BUN_BROWSER_HOME = DAEMON_DIR;
...
describe("ensureDaemon passes CDP info to daemon", () => {
  ...
  it("daemon discovers CDP via managed port file", async () => {
    // Daemon should read ~/.bun-browser/browser/cdp-port and find our fake CDP
...
describe("core bug: daemon spawned without --cdp-port fails silently (#136)", () => {
  ...
  it("daemon with default port 19825 and nothing listening → exits, no daemon.json", async () => {
    ...
    const child = spawn(TSX, [DAEMON_ENTRY], {
      detached: true,
      stdio: "ignore",
      env: { ...process.env, BUN_BROWSER_CDP_URL: undefined },
    });
    child.unref();
...
// Requires fast CDP WebSocket failure; skipped by default under Bun's test runner.
const describe503 = process.env.BUN_BROWSER_CDP503_TESTS ? describe : describe.skip;

describe503("CDP 503 error includes diagnostics", () => {
  let daemonPid: number | null = null;
  ...
  it("503 response includes CDP target, reason, and hint — responds immediately not 30s", { timeout: 15000 }, async () => {
    const cdpPort = 39989;
