# bun-browser Harness Test Scenarios

这些场景用于验证 bun-browser 对 AI Agent 的可用性。每次大改后应该让 Agent 执行这些场景。

## 前提条件

- Chrome 已启动并开启 CDP: `--remote-debugging-port=19222`
- Daemon 已运行: `bun-browser-daemon --cdp-port 19222`
...
用 AI Agent（Claude Code、Codex、Gemini CLI 等）执行以上场景，使用 bun-browser daemon HTTP API。
...
# 启动 daemon
cd /path/to/bun-browser
pnpm --filter @bun-browser/daemon build
...
**步骤：**
1. 启动 daemon，确认 `~/.bun-browser/daemon.json` 存在
2. `kill -9 <daemon-pid>` 强制杀进程
3. 确认 `daemon.json` 残留（未被正常清理）
4. 运行 `bun-browser eval "1+1"` — CLI 应检测到 PID 不存活，删除旧文件，重新 spawn daemon
5. 验证命令正常返回结果
