/**
 * bun-browser Daemon — CDP-direct backend
 *
...
const DAEMON_DIR = process.env.BUN_BROWSER_HOME || path.join(os.homedir(), ".bun-browser");
...
function parseOptions(): DaemonOptions {
  ...
  if (values.help) {
    console.error(`
bun-browser-daemon — CDP-direct backend for bun-browser
