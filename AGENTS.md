...
bun-browser 有两类用户：**人类**（直接用 CLI）和 **AI Agent**（通过 bash/MCP 调用）。Agent 是桥梁 — 读取 bun-browser 输出并翻译给人类。每个文本表面都要同时服务两者。
...
- 构建：在仓库根目录执行 `bun run build`
- site adapter 不需要写测试

## 测试

- `bun test` — 运行所有测试（通过 turbo 编排）
- `bun run build` — 类型安全检查（提交前必须运行）
- `bun run lint` — 代码风格（daemon, mcp, shared）
