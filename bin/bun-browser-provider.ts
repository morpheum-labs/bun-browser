/**
 * bun-browser-provider — Edge Clip provider for Pinix Hub
 *
...
const LOG_PREFIX = "[bun-browser-provider]";
const DEFAULT_HUB_URL = "http://127.0.0.1:9000";
const PROVIDER_NAME = "bun-browser";
...
function parseArgs(argv: string[]): string {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (arg === "--hub-url" || arg === "--pinix-url") {
      ...
      console.log(`Usage: bun-browser-provider [--hub-url <url>]\n\nOptions:\n  --hub-url <url>  Hub gRPC URL (default: ${DEFAULT_HUB_URL})`);
      process.exit(0);
    } else {
